...
public class ServiceRequestService: IServiceRequestService{
    ...
    // DI container injects both dependencies automatically.
    // ServiceRequestService has no knowledge of SignalR.
    ...
    public async Task<(bool ok, string error, ServiceRequestDto? created)> CreateAsync(CreateServiceRequestDto dto) {
        var (ok, error) = ServiceRequestValidators.Validate(dto);
        ...
        await _notifier.NotifyRequestCreatedAsync(); // Notify real-time clients
        return (true, "", ToDto(created));
    }

    public async Task<(bool ok, string error,ServiceRequestDto? updated)> UpdateAsync(int id, UpdateServiceRequestDto dto)  {
        var (ok, error) = ServiceRequestValidators.Validate(dto);
        if (!ok) return (false, error, null);
    ...
    // ── Private helper ──────────────────────────────
    private static ServiceRequestDto ToDto(ServiceRequest e) => new ServiceRequestDto(
            e.Id,
            e.Title,
            e.Description,
            (int)e.Status,
